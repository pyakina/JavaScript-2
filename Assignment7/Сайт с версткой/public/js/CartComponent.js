...
Vue.component('cart', {
    ...
    <button class="cart-btn" type="button" @click="showCart = !showCart"><svg class="cart-btn-icon" width="32" height="29" viewBox="0 0 32 29" xmlns="http://www.w3.org/2000/svg">
  ...
  </svg><span class="cart-btn-txt">{{ this.cartItems.reduce((summ, item) => summ + item.quantity, 0) }}</span></button>
    <h2 v-if=" cartItems.length === 0">Корзина пуста</h2>
        <div v-else >
           
            <div class="cart-block" v-show="showCart">      
                <h3 > {{ this.cartItems.reduce((summ, item) => summ + item.quantity, 0) }} товара(ров) Итого: $ {{(this.cartItems.reduce((summ, item) => summ + item.quantity*item.price, 0)).toFixed(2)}}</h3>
...
Vue.component('cart-item', {
    ...
    <div class="cart-item">
        ...
        <div class="product-desc">
            ...
            <p>Сумма: $ {{(cartItem.price * cartItem.quantity).toFixed(2) }}</p>
        </div>
    </div>
    `
})